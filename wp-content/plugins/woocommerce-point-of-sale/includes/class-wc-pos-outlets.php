...
class WC_Pos_Outlets{
	...
	public function display() {
    	...
    	<form method="get" action="" class="search-form">
				<p class="search-box">
					<label for="outlet-search-input" class="screen-reader-text"><?php _e( 'Search Outlets', 'wc_point_of_sale' ); ?></label>
					<input type="hidden" value="wc_pos_outlets" name="page">
					<input type="search" value="" name="s" id="outlet-search-input">
					<input type="submit" value="<?php _e( 'Search Outlets', 'wc_point_of_sale' ); ?>" class="button" id="search-submit" name="">
				</p>
  	...
  	<?php endif; ?>
					...
					<tbody>
						<tr class="form-field form-required">
							...
							<td>
								<?php if($data['name']){ ?>
									<input type="text" aria-required="true" size="40" id="_outlet_name" name="_outlet_name" value="<?php echo $data['name']; ?>" required>
								<?php }else{ ?>
									<input type="text" aria-required="true" size="40" id="_outlet_name" name="_outlet_name" required>
								<?php } ?>
								<p class="description"><?php _e( 'The name of the outlet as it appears when opening a register.', 'wc_point_of_sale' ); ?><p>				
							</td>
						</tr>
						<?php 
					...
					<p class="submit"><input type="submit" value="<?php _e( 'Update', 'wc_point_of_sale' ); ?>" class="button button-primary" id="submit" name="submit"></p>
			<?php if(!$ajax) : ?>
    	</form>
  	</div>
  <?php endif; ?>
