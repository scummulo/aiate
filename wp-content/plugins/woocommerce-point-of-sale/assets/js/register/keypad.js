    ...
    calculateSelectedPrice = function () {
        ...
        $('#order_items_list .new_row .product_price').keypad({
            ...
            beforeShow: function (div, inst, value) {
                ...
                $('<div class="keypad-row"><div class="disc-type pound active button keypad-key" data-value="0">' + wc_pos_params.currency_format_symbol + '</div><input type="text" id="discount-value" data-row="0" value="' + input_val + '"><div class="disc-type percents button keypad-key" data-value="0">%</div><div class="button restore-price"></div></div>').prependTo(div);
                jQuery('#discount-value').select();
                hideEmptyDiscounts();
            },
            onClose: function (value, inst) {
                var row_id = jQuery('#discount-value').data('row');
                var price_input = jQuery('#' + row_id).find('.product_price');
