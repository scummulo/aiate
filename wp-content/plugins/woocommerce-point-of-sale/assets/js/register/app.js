...
// ajaxSetup is global, but we use it to ensure JSON is valid once returned.
    ...
    resizeCart = function () {
        $("#order_items_list-wrapper").hide();
        var h = $('#order_items_list').closest('#woocommerce_order_items-container').height();
        var b_height = $('#wc-pos-register-buttons').height();
        if ($('#order_items_list-wrapper').length) {
            $('#order_items_list-wrapper').height(h - b_height + 30);
        } else {
            var wrap = $('<div id="order_items_list-wrapper"></div>').height(h - b_height + 30);
            $('#order_items_list').closest('table').wrap(wrap);
        }
        $("#order_items_list-wrapper").show();
    }
    function resizeGrid() {
        if (pos_grid.second_column_layout == 'product_grids') {
